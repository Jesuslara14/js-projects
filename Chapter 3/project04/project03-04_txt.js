...
function loadReviews() {
      ...
      reviewers.forEach((reviewer, index) => {
            ...
            <table class='${reviewType[index] === 'P'? 'prime' : reviewType[index] === 'N' ? "new" : ''}'>
                  <tr><th>By</th><td>${reviewer}</td></tr> 
                  <tr><th>Review Date</th><td>${reviewDates[index]}</td></tr> 
                  <tr><th>${imageStars(stars[index])}</th></tr>
...
function imageStars(rating){
      let starText = '';

      for(let i = 0; i < rating; i++){
            starText += `<img src='star.png' />`
      }
